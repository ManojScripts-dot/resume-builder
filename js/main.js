...
function handleError(fn, context = "") {
  return function (...args) {
    try {
      ...
      showModal(
        "Error",
        `An error occurred in ${context}. Please check the console for details.`,
        null
      );
    }
  };
      ...
      (proj) => `
        <div class="project-item" data-id="${proj.id}">
            ...
            <input type="text" placeholder="Website" value="${
              proj.website
            }" onchange="updateProject(${proj.id}, 'website', this.value)">
            <input type="text" placeholder="Source Code" value="${
              proj.sourceCode
            }" onchange="updateProject(${proj.id}, 'sourceCode', this.value)">
            <input type="text" placeholder="Technologies Used" value="${
              proj.technologies
                ...
                ${proj.points
                  .map(
                    ...
                    <div class="point-input-group">
                        <input type="text" placeholder="Project Point ${
                          index + 1
                        }" value="${point}" onchange="updateProjectPoint(${
                      proj.id
                    }, ${index}, this.value)">
                        <button class="btn secondary btn-small" onclick="removeProjectPoint(${
                          proj.id
                        }, ${index})"><i class="fas fa-times"></i></button>
                    </div>
                `
                  )
                  .join("")}
            </div>
            <button class="btn secondary" onclick="addProjectPoint(${
              proj.id
            })">Add Point</button>
            <button class="btn secondary" onclick="removeProject(${
              proj.id
    ...
    .map(
      (vol) => `
        <div class="volunteer-item" data-id="${vol.id}">
            <input type="text" placeholder="Organization" value="${
              vol.organization
            }" onchange="updateVolunteer(${
        vol.id
      }, 'organization', this.value)">
            <input type="text" placeholder="Period" value="${
              vol.period
            }" onchange="updateVolunteer(${vol.id}, 'period', this.value)">
            <input type="text" placeholder="Role" value="${
              vol.role
            }" onchange="updateVolunteer(${vol.id}, 'role', this.value)">
            <input type="text" placeholder="Location" value="${
              vol.location
            }" onchange="updateVolunteer(${vol.id}, 'location', this.value)">
                  ...
                  .map(
                    ...
                    <div class="point-input-group">
                        <input type="text" placeholder="Responsibility ${
                          index + 1
                        }" value="${resp}" onchange="updateVolunteerResponsibility(${
                      vol.id
                    }, ${index}, this.value)">
                        <button class="btn secondary btn-small" onclick="removeVolunteerResponsibility(${
                          vol.id
                        }, ${index})"><i class="fas fa-times"></i></button>
                    </div>
                `
            ...
            <button class="btn secondary" onclick="addVolunteerResponsibility(${
              vol.id
            })">Add Responsibility</button>
            <button class="btn secondary" onclick="removeVolunteer(${
              vol.id
            })">Remove</button>
        </div>
    `
    ...
    .map(
      (skill, index) => `
        <div class="soft-skill-item">
            <input type="text" placeholder="Soft Skill Point ${
              index + 1
            }" value="${skill}" onchange="updateSoftSkill(${index}, this.value)">
...
function goToStep(stepNumber) {
  ...
  for (let i = 1; i <= stepNumber; i++) {
    document
      .querySelector(`.step-dot[data-step="${i}"]`)
...
function setupEventListeners() {
  try {
    ...
    document.getElementById("finalizeBtn").addEventListener("click", () => {
      showModal(
        "Resume Complete",
        "Your resume is complete! You can now download it as a PDF.",
        () => {
          downloadPDF();
        }
      );
    ...
    if (newResumeBtn) {
      newResumeBtn.addEventListener("click", () => {
        showModal(
          "Start New Resume",
          "Are you sure you want to start a new resume? This will clear all current data.",
          () => {
            state = {
              personalInfo: {
                fullName: "",
            ...
            showModal("New Resume", "Started a new resume!", null);
          }
        );
      });
    }
    ...
    showModal(
      "Error",
      "Error setting up the application. Please refresh the page.",
      null
    );
  }
}
    ...
    .map(
      (edu) => `
        <div class="education-item" data-id="${edu.id}">
            ...
            <input type="text" placeholder="Level (e.g., Bachelor's, Master's)" value="${
              edu.level || ""
            }" onchange="updateEducation(${edu.id}, 'level', this.value)">
            <div class="ongoing-checkbox">
                <label>
                    <input type="checkbox" ${
                      edu.isOngoing ? "checked" : ""
                    } onchange="updateEducation(${
        edu.id
      }, 'isOngoing', this.checked)"> 
                    Currently Studying
                </label>
            </div>
            ${
              edu.isOngoing
                ? `<input type="text" placeholder="Expected Graduation (e.g., May 2025)" value="${
                    edu.expectedGraduation || ""
                  }" onchange="updateEducation(${
                    edu.id
                  }, 'expectedGraduation', this.value)">`
            ...
            <div class="gpa-input">
                ...
                <input type="text" placeholder="Relevant Courses" value="${
                  edu.courses
                }" onchange="updateEducation(${edu.id}, 'courses', this.value)">
            </div>
            <button class="btn secondary" onclick="removeEducation(${
              edu.id
            })">Remove</button>
        </div>
    `
...
function generatePDF() {
  try {
    ...
    setTimeout(() => {
      const opt = {
        margin: [0, 0, 0, 0],
        filename: `${state.personalInfo.fullName.replace(
          /\s+/g,
          "_"
        )}_resume.pdf`,
...
function downloadPDF() {
  try {
    if (!state.personalInfo.fullName) {
      showModal(
        "Error",
        "Please enter your name before downloading the PDF.",
        null
      );
      return;
    }
    ...
    if (typeof window.html2pdf === "undefined") {
      console.error(
        "html2pdf is not defined. Attempting to load the library..."
      );
    ...
    preview.innerHTML = `
            <div class="resume-header">
                ...
                <div class="contact-info">
                    ${
                      sanitizedState.personalInfo.phone
                        ? `<span><i class="fas fa-phone"></i> ${sanitizedState.personalInfo.phone}</span>`
                        : ""
                    }
                    ${
                      sanitizedState.personalInfo.email
                        ? `<span><i class="fas fa-envelope"></i> <a href="mailto:${sanitizedState.personalInfo.email}">${sanitizedState.personalInfo.email}</a></span>`
                        : ""
                    }
                    ${
                      sanitizedState.personalInfo.github
                        ? `<span><i class="fab fa-github"></i> <a href="${
                            sanitizedState.personalInfo.github
                          }">${sanitizedState.personalInfo.github.replace(
                            "https://github.com/",
                            ""
                          )}</a></span>`
                        : ""
                    ...
                    ${
                      sanitizedState.personalInfo.portfolio
                        ? `<span><i class="fas fa-briefcase"></i> <a href="${
                            sanitizedState.personalInfo.portfolio
                          }">${sanitizedState.personalInfo.portfolio.replace(
                            "https://",
                            ""
                          )}</a></span>`
                        : ""
                      ...
                      .map(
                        ...
                        <div class="education-item">
                          ...
                          edu.gpa ? ` - ${edu.gpa}` : ""
                            ...
                            ${
                              edu.courses
                                ? `<div class="courses-line">Courses: ${edu.courses}</div>`
                                : ""
                            }
                        </div>
                    `
            ...
            ${
              Object.values(sanitizedState.technicalSkills).some(
                (value) => value
              )
                ? `
                <section>
                    ...
                    ${sanitizedState.projects
                      .map(
                        (proj) => `
                          ...
                          proj.sourceCode
                            ? `| <a href="${proj.sourceCode}">Source Code</a>`
                            : ""
                        }</span>
...
function saveResume() {
  try {
    ...
    showModal("Save Resume", "Resume saved successfully!", null);
  } catch (error) {
    console.error("Error saving resume:", error);
    showModal("Error", "Failed to save resume. Please try again.", null);
  }
}
